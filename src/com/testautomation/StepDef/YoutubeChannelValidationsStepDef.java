...
//	YoutubeSearchPage ysp = new YoutubeSearchPage(driver);
	...
	public void login_Spyglass() throws Throwable
	{
		ExtentTest logInfo=null;
		try {
			test = extent.createTest(Feature.class, "Youtube channel name validation");							
			test=test.createNode(Scenario.class, "Login Spy Glass");						
			logInfo=test.createNode(new GherkinKeyword("Given"), "Login to SpyGlass Apllication");
			Properties properties=obj.getProperty(); 		
			driver=BrowserUtility.OpenBrowser(driver, properties.getProperty("browser.name"), properties.getProperty("browser.spyGlass"));
			YoutubeSearchPage ysp = new YoutubeSearchPage(driver);
			ysp.Login();
			logInfo.pass("Logged into SpyGlass application successfully");
			logInfo.addScreenCaptureFromPath(captureScreenShot(driver));			
	...
	public void logout() throws Throwable
	{
		ExtentTest logInfo=null;
		try {									
			logInfo=test.createNode(new GherkinKeyword("Then"), "Verify login user name and logout");
			String expectedChannelName="Spyglass";
			String actualChannelName=new YoutubeChannelPage(driver).getTitle();
			Assert.assertEquals(actualChannelName, expectedChannelName,"Channel names are not matching");
